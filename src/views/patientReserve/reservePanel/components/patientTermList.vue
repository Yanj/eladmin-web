...
<script>
import patientTermApi from '@/api/patientReserve/patientTerm'
  ...
  methods: {
    ...
    loadPatientList() {
      ...
      if (this.query.patientId) {
        patientTermApi.getAllPatientTerms(this.query).then(res => {
          console.log(res)
          if (res.content) {
            this.patientTerms = res.content
            if (res.content.length > 0) {
              this.currentPatientTerm = res.content[0]
